...
class XianyuReplyBot:
    ...
    def update_prompts(self, custom_prompts: Dict[str, str]):
        """
        使用自定义提示词更新Agent
        
        Args:
            custom_prompts: 自定义提示词字典，key为提示词类型，value为提示词内容
        """
        logger.info("正在更新自定义提示词...")
        
        # 更新提示词
        if 'classify' in custom_prompts:
            self.classify_prompt = custom_prompts['classify']
        if 'price' in custom_prompts:
            self.price_prompt = custom_prompts['price']
        if 'tech' in custom_prompts:
            self.tech_prompt = custom_prompts['tech']
        if 'default' in custom_prompts:
            self.default_prompt = custom_prompts['default']
        
        # 重新初始化Agent
        self._init_agents()
        self.router = IntentRouter(self.agents['classify'])
        
        logger.info(f"自定义提示词更新完成，更新了 {len(custom_prompts)} 个提示词")
