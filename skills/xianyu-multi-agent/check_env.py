...
def check_env_config():
    ...
    print(f"   OpenClaw CLI: {'✅ 已检测到（可直接走 openai-codex/gpt-5.4）' if has_openclaw else '❌ 未检测到'}")
    ...
    if not coze_api_key or not coze_bot_id:
        print("⚠️  扣子配置不完整（买家智能体可选，未配置时会走本机 OpenClaw / 备用模型）")
    
    if api_key_has_issue or bot_id_has_issue or base_url_has_issue:
        print("❌ 扣子配置格式有问题（包含注释或特殊字符）")
    
    if not has_openclaw and not api_key:
        print("❌ 未检测到 openclaw，也未配置 API_KEY")
        has_issues = True
        has_issues = True
