...
class CozeClient:
    ...
    def __init__(self):
        ...
        if not self.api_key or not self.buyer_bot_id:
            logger.warning("扣子API配置不完整，将使用 OpenClaw / OpenAI 备用客户端")
            self.fallback_client = XianyuLLM()
        else:
    ...
    def _fallback_chat(self, messages):
        ...
        try:
            content = self.fallback_client.complete_text(
                messages,
                temperature=0.8,
                max_tokens=200,
                top_p=0.8,
            ...
            logger.error(f"Fallback 模型调用失败: {e}")
            return "嗯，我想想"
