...
- 2026-03-28 已明确：公众号排版与推草稿箱固定回退并锁定到旧稳定链路 `skills/wechat-article-formatter/` + `scripts/wechat_draft.py`，不再对公众号任务临时切换未验收的新 skill；默认母版以已验证文章《宠物AI陪伴》为准：橙韵风格、主色 `#fb923c`、深橙 `#ea580c`、强调色 `#d97706`、结构为“渐变头图→白色hook卡→中文序号章节→浅黄引用/表格卡片”；当次验收稿题为《别再让云AI偷走你的用户信任了》，草稿箱推送成功，MediaID:`d59as12_SarRDV5X0i_GflbmGqzxEgQBO67qk4usFPdVMlkGGxmXXfAqlKyx_V9W`，统一排版稿已落 `drafts/2026-03-28_别再让云AI偷走你的用户信任了_橙韵排版.html`。
...
- `skills/ai-invest-agent/`：手动调用的投资复盘技能包，入口看`README.md`与`prompts/weekly_review.md`/`prompts/quick_check.md`；覆盖A股/港股/美股四步复盘、33指数温度计，支持MD/DOCX/XLSX输出。
...
- 2026-03-28 微信/Telegram 体感明显慢于网页端，实查主因不是通道掉线，而是旧 channel session 上下文过重：网页端约 26k tokens，而 Telegram 约 88k、微信约 140k；处理方式是备份后清理 `~/.openclaw/agents/main/sessions/sessions.json` 中对应 `agent:main:telegram:direct:8334640972` 与 `agent:main:openclaw-weixin:direct:o9cq801zz_bpl-yrmyssbyld3jes@im.wechat` 条目，让下一条消息重建轻会话。验收标准：两端新 direct session 重建到约 24k tokens，首条回复恢复正常速度。
- 同日 guardian 曾持续报 `validateConfigObjectWithPlugins export not found`，但根因不是函数不存在，而是 runtime validator 模块解析错位；已确认当前 OpenClaw `dist/config-*.js` 中真实存在 `validateConfigObjectWithPlugins` / `validateConfigObjectRawWithPlugins`，并在实际运行入口 `~/.openclaw/guardian_runtime/scripts/openclaw_guardian.py` 将 validator 模块钉到正确的 `config-*.js` 后恢复。验收标准：`python3 ~/.openclaw/guardian_runtime/scripts/openclaw_guardian.py check-once --verbose` 返回 healthy，且 `~/.openclaw/logs/guardian.log` 不再新增该 export 报错。
- 2026-03-24 小红书以后默认必须走 v2 路径做文案与出图：`skills/Auto-Redbook-Skills` 的 `v2/strategy + v2/render + v2/themes + v2/image_ai`；旧链路只保留兼容/兜底。稳定真发入口不是 `Auto-Redbook-Skills` 自带旧版 `publish_xhs.py`（默认吃 `.env` 里的 `XHS_COOKIE`），而是工作区技能包 `skills/小红书笔记技能包/scripts/publish_xhs.py` 的 `--payload --browser-mode --browser-profile-dir ~/xhs_workspace/xiaohongshu-send/profile-persistent --cookies-path ~/xhs_workspace/xiaohongshu-send/data/cookies.json` 链路。payload 中 `content` 必须优先作为正文；默认正文结构采用本次验证通过的丰满版：痛点开头→筛选标准→典型场景→如何开始用→互动提问。首条真实验证选“清单型”最稳：封面主标题 `5个AI工具直接抄`；重推时优先只改标题和 desc，不动图片，且继续 `仅自己可见` 验证。若真实结果暴露封面遮挡/页重复/文字不够活跃，优先在 `v2/render/page_renderer.py` 调整清单型封面元素位置，再对样例 `note_plan` 做定制页文案（避免自动模板重复），最后再重发；若只剩正文偏短，继续只改 payload 文案，不再动图片。
## 7. 记忆规则
