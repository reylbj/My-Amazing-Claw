...
- 2026-03-24 小红书当前稳定真发入口不是 `Auto-Redbook-Skills` 自带旧版 `publish_xhs.py`（默认吃 `.env` 里的 `XHS_COOKIE`），而是工作区技能包 `skills/小红书笔记技能包/scripts/publish_xhs.py` 的 `--payload --browser-mode --browser-profile-dir ~/xhs_workspace/xiaohongshu-send/profile-persistent --cookies-path ~/xhs_workspace/xiaohongshu-send/data/cookies.json` 链路。首条真实验证选“清单型”最稳：封面主标题 `5个AI工具直接抄`；重推时优先只改标题和 desc，不动图片，且继续 `仅自己可见` 验证。若真实结果暴露封面遮挡/页重复/文字不够活跃，优先在 `v2/render/page_renderer.py` 调整清单型封面元素位置，再对样例 `note_plan` 做定制页文案（避免自动模板重复），最后再重发；若只剩正文偏短，继续只改 payload 文案，不再动图片。
## 7. 记忆规则
