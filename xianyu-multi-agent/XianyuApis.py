...
class XianyuApis:
    ...
    def clear_duplicate_cookies(self):
        ...
        before_cookies = [cookie.name for cookie in self.session.cookies]
        ...
        for cookie in cookie_list:
            ...
            if cookie.name not in added_cookies:
                ...
                logger.warning(f"🔍 [DEBUG] 保留cookie: {cookie.name}={cookie.value[:20]}...")
            else:
        ...
        after_cookies = [cookie.name for cookie in self.session.cookies]
    ...
    def hasLogin(self, retry_count=0):
        ...
        try:
            ...
            logger.warning(f'🔍 [DEBUG] hasLogin response.cookies: {self.session.cookies}')
