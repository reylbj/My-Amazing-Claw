...
def decode_messages(msgs: List[Dict]) -> List[Dict]:
    """
    解密消息列表
    
    Args:
        msgs: 消息列表，每个消息可能包含加密的内容
        ...
        try:
            ...
            if 'content' in msg and isinstance(msg['content'], str):
                try:
                    ...
                    # 尝试解析为JSON
                    decoded_msg['content'] = json.loads(decrypted_content)
                except:
                    # 如果解密失败，保持原内容
                    decoded_msg['content'] = msg['content']
            
            # 检查其他可能需要解密的字段
            for field in ['body', 'data', 'message']:
                if field in msg and isinstance(msg[field], str):
                    try:
                        decrypted_field = decrypt(msg[field])
                        decoded_msg[field] = json.loads(decrypted_field)
                    except:
                        # 如果解密失败，保持原内容
                        decoded_msg[field] = msg[field]
